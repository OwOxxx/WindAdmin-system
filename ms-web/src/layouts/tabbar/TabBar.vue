...
<template>
  <div class="tab-bar">
    <el-tabs v-model="activeTableTabsValue" type="card" class="tabbar" @tab-remove="removeTab" @tab-change="changeTab">
      <el-tab-pane v-for="item in visitedViewStore.visitedViewList" :key="item.name" :label="item.title" :name="item.name"
        :closable="item.title !== '主控台'">
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
